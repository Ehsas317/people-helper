...
def generate_report(
    ...
    lines.append("**Scoring:** 0.5 x code quality + 0.3 x uniqueness + 0.2 x demand signal")
    lines.append("")
    ...
    if not active:
        ...
        for i, c in enumerate(active[:max_candidates], 1):
            ...
            lines.append(f"**Code quality:** {c.code_quality:.1f}/10 (weight: 50%)")
            lines.append(f"**Uniqueness:** {c.uniqueness:.1f}/10 (weight: 30%)")
            lines.append(f"**Demand signal:** {c.demand_signal:.1f}/10 (weight: 20%)")
            ...
            if c.complexity > 0:
                cc_label = (
                    "low (simple)" if c.complexity <= 5
                    else "moderate" if c.complexity <= 10
                    else "high (consider refactoring)" if c.complexity <= 20
                    else "very high (god function)"
                )
                lines.append(f"**Cyclomatic complexity:** {c.complexity} — {cc_label}")
            if c.fan_in >= 0:
                lines.append(
                    f"**Fan-in:** {c.fan_in}"
                    + (" (orphan — ideal extraction target)" if c.fan_in == 0 else "")
                )
            if c.in_cycle:
                lines.append("**Import cycle:** ⚠ yes — extraction requires breaking the cycle first")
            lines.append("")
    ...
    lines.append("---\n")
    lines.append("*Generated by People Helper v0.3 — Read-only by design.\n")
