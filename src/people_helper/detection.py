...
from .config import (
    ...
    FRAMEWORK_SPECIAL_FILES, FRAMEWORK_DIRS,
...
def has_test_for(file_path: str, all_files: set) -> bool:
    ...
    bare_names = [f"test_{stem}{ext}", f"{stem}_test{ext}", f"{stem}Test{ext}",
...
def _resolve_sibling(sib_name: str, parent_level: int, file_path: str, ext: str, file_set: set) -> str | None:
    ...
    if ext == ".py":
        if target_str == ".":
            ...
            candidates = [str(target_dir / f"{sib_name}.py"), str(target_dir / sib_name / "__init__.py")]
    elif ext in {".ts", ".tsx", ".js", ".jsx"}:
        if target_str == ".":
            candidates = [f"{sib_name}.ts", f"{sib_name}.tsx", f"{sib_name}.js",
                          f"{sib_name}.jsx", f"{sib_name}/index.ts", f"{sib_name}/index.tsx",
                          f"{sib_name}/index.js", f"{sib_name}/index.jsx"]
        else:
            candidates = [str(target_dir / f"{sib_name}.ts"), str(target_dir / f"{sib_name}.tsx"),
                          str(target_dir / f"{sib_name}.js"), str(target_dir / f"{sib_name}.jsx"),
                          str(target_dir / sib_name / "index.ts"), str(target_dir / sib_name / "index.tsx"),
                          str(target_dir / sib_name / "index.js"), str(target_dir / sib_name / "index.jsx")]
    elif ext == ".rs":
        if target_str == ".":
            candidates = [f"{sib_name}.rs", f"{sib_name}/mod.rs"]
        else:
            candidates = [str(target_dir / f"{sib_name}.rs"), str(target_dir / sib_name / "mod.rs")]
    for c in candidates:
        if c in file_set:
            return c
    return None
...
_LICENSE_FILENAMES = {
    ...
    "license-mit", "license.mit", "license_apache", "license.apache",
    "license-apache", "license-2.0.txt", "license-2.0",
    "copying", "copying.txt", "copying.less", "copying.lesser",
    "unlicense", "unlicense.txt",
    "notice", "notice.md", "copyright", "copyright.txt",
    ...
    def strongconnect(v):
        ...
        if lowlink[v] == index[v]:
            ...
            while True:
                ...
                if w == v: break
            if len(scc) >= 2: result.append(scc)
    old = sys.getrecursionlimit()
    if len(graph) > 500: sys.setrecursionlimit(max(old, len(graph) * 2 + 100))
    try:
...
def _realism_check(content: str, ext: str, path: str, loc: int) -> str | None:
    lines = content.splitlines()
    ...
    actual_copyright_lines = sum(
        1 for l in lines[:30]
        if "copyright" in l.lower() or "licensed under" in l.lower()
        or "apache license" in l.lower() or "mit license" in l.lower()
...
def _build_why_extractable(cand, fan_in: int, in_cycle: bool) -> list:
    reasons = []
    if cand.has_tests: reasons.append("Has corresponding test file")
    if cand.has_docstring: reasons.append("Has module-level documentation")
    if cand.internal_imports == 0: reasons.append("Zero internal project imports — fully self-contained")
    elif cand.internal_imports == 1: reasons.append("Only 1 internal import — loosely coupled")
    if cand.external_imports <= 3: reasons.append(f"Small dependency footprint ({cand.external_imports} external import(s))")
    if cand.loc < 50: reasons.append(f"Very small ({cand.loc} LOC)")
    elif cand.loc < 150: reasons.append(f"Manageable size ({cand.loc} LOC)")
    if cand.filename_score > 0: reasons.append("Filename suggests a reusable utility")
    if fan_in == 0: reasons.append("Zero fan-in (orphan) — nothing else depends on it")
    elif fan_in == 1: reasons.append(f"Low fan-in ({fan_in} importer)")
    if in_cycle: reasons.append("⚠ Part of an import cycle — extraction requires breaking the cycle first")
    if 0 < cand.complexity <= 5: reasons.append(f"Low cyclomatic complexity (cc={cand.complexity})")
    elif cand.complexity > 15: reasons.append(f"⚠ High cyclomatic complexity (cc={cand.complexity})")
    if cand.is_stdlib_only: reasons.append("Stdlib-only — zero external dependencies, truly standalone")
    elif cand.dependency_weight >= 3: reasons.append(f"⚠ Heavy dependencies (weight={cand.dependency_weight})")
    if cand.api_surface_count >= 3: reasons.append(f"Rich API surface ({cand.api_surface_count} public functions/classes)")
    elif cand.api_surface_count == 1: reasons.append(f"⚠ Only 1 public function — snippet, not a full library")
    if cand.has_project_specific_refs: reasons.append("⚠ References framework internals")
    if cand.comment_ratio >= 0.15: reasons.append(f"Well-commented ({cand.comment_ratio:.0%} comment lines)")
    for name in cand.function_names:
        if name and any(p in name.lower() for p in ["slugify", "validate", "parse", "format", "encode", "decode", "hash", "cache", "sort", "filter", "search", "compress", "encrypt"]):
...
def detect_candidates(files: list, primary_language: str) -> list:
    ...
    for f in files:
        if f["ext"] != ".py": continue
        p = Path(f["path"])
        if len(p.parts) == 1: project_modules.add(p.stem)
        else:
            project_modules.add(p.parts[0])
            if p.name == "__init__.py": project_modules.add(p.parent.name)
    ...
    for f in files:
      try:
        ...
        if any(p in {"tests", "test", "__tests__", "test_utils", "testfixtures", "fixtures"} for p in path_parts_lower): continue
        stem = Path(path_str).stem.lower()
        if any(x in stem for x in ["test_", "_test", ".test", ".spec", "conftest"]): continue
        if Path(path_str).name in {"__init__.py", "conftest.py"}: continue

        content = f["content"]
        loc = handler.count_loc(content)
        if loc < 10 or loc > 500: continue
        if is_framework_route(path_str): continue

        skip_reason = _realism_check(content, ext, path_str, loc)
        if skip_reason:
            candidates.append(Candidate(path=path_str, language=lang, loc=loc,
                has_tests=False, has_docstring=False, internal_imports=0,
                external_imports=0, filename_score=0.0, skipped=True, skip_reason=skip_reason,
        ...
        if internal >= 2:
            candidates.append(Candidate(path=path_str, language=lang, loc=loc,
                has_tests=tested, has_docstring=has_doc, internal_imports=internal,
                external_imports=external, filename_score=fscore, skipped=True,
        ...
        if fscore < -0.5:
            candidates.append(Candidate(path=path_str, language=lang, loc=loc,
                has_tests=tested, has_docstring=has_doc, internal_imports=internal,
                external_imports=external, filename_score=fscore, skipped=True,
                skip_reason="Filename is a framework entry point", source_has_license=repo_has_license))
            continue
        ...
        if missing_siblings:
            candidates.append(Candidate(path=path_str, language=lang, loc=loc,
                ...
                relative_imports=[n for n, _ in relative_imports], missing_siblings=missing_siblings,
                extraction_type="blocked", source_has_license=repo_has_license))
            continue
        ...
        cand = Candidate(
            ...
            complexity=complexity, fan_in=file_fan_in, in_cycle=in_cycle,
            dependency_weight=dep_weight, api_surface_count=api_count,
            is_stdlib_only=is_stdlib, has_project_specific_refs=has_proj_refs,
            function_names=func_names, comment_ratio=comment_ratio,
            relative_imports=[n for n, _ in relative_imports], sibling_paths=sibling_paths,
            extraction_type=extraction_type, source_has_license=repo_has_license,
        )
        cand.why_extractable = _build_why_extractable(cand, file_fan_in, in_cycle)
        candidates.append(cand)
      except Exception:
        # Skip files that cause unexpected errors (rare edge cases)
        continue
    return candidates
