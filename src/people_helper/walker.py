...
from .config import LANG_BY_EXT, SKIP_DIRS, SKIP_EXTS, GENERATED_PATTERNS

# Content-based patterns that indicate a file is generated.
# Checked against the first 500 chars of file content.
GENERATED_CONTENT_PATTERNS = [
    re.compile(r"(?i)code generated by"),
...
def walk_repo(root: Path) -> list:
    ...
    for path in root.rglob("*"):
        ...
        if path.name.startswith(".") and not path.name.endswith(".example"):
            continue
        # Skip generated/auto-generated files (e.g. zz_generated.*, *_generated.*)
        rel_str = str(rel).lower()
        if any(pattern in rel_str for pattern in GENERATED_PATTERNS):
            continue
        ...
        if content.startswith("\ufeff"):
            content = content[1:]
        # Content-based generated file detection (catches files like alldocs.go
        # whose filename doesn't match patterns but header says "Code generated by")
        header = content[:500]
        if any(pat.search(header) for pat in GENERATED_CONTENT_PATTERNS):
            continue
