...
_SWIFT_STDLIB = {"Foundation", "UIKit", "SwiftUI", "Combine", "CoreData",
                 "CoreGraphics", "CoreImage", "CoreLocation", "CoreText",
                 "MapKit", "WebKit", "CloudKit", "StoreKit", "AVFoundation",
                 "AVKit", "AudioToolbox", "CoreAudio", "CoreMedia",
                 "CoreVideo", "CoreAnimation", "AppKit", "Security",
                 "CryptoKit", "Network", "OSLog", "XCTest", "Swift",
                 "Dispatch", "Metal"}
...
class SwiftHandler(LanguageHandler):
    ...
    def count_public_api(self, content: str) -> tuple:
        ...
        for m in re.finditer(r"^\s*(?:public\s+|open\s+|internal\s+|fileprivate\s+|private\s+)?func\s+(\w+)", content, re.MULTILINE):
            name = m.group(1)
            # Skip private funcs (don't count)
            if "private" in m.group(0):
                continue
            count += 1; names.append(name)
        for m in re.finditer(r"^\s*(?:public\s+|open\s+|internal\s+|fileprivate\s+|private\s+)?(?:struct|class|enum|protocol)\s+(\w+)", content, re.MULTILINE):
            if "private" in m.group(0):
                continue
