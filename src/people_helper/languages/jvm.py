    ...
    def count_public_api(self, content: str) -> tuple:
        ...
        if self.is_kotlin:
            ...
            for m in re.finditer(
                r"^\s*(?:public\s+|open\s+|final\s+|abstract\s+|sealed\s+)*(?:fun|override\s+fun)\s+(\w+)",
                content,
                re.MULTILINE,
            ):
                count += 1
            ...
            for m in re.finditer(
                r"^\s*(?:public\s+|open\s+|final\s+|abstract\s+|sealed\s+)*(?:class|object|interface|enum\s+class|data\s+class)\s+(\w+)",
                content,
                re.MULTILINE,
            ):
                count += 1
            ...
            for m in re.finditer(
                r"^\s*public\s+(?:final\s+|abstract\s+|static\s+)*(?:class|interface|enum)\s+(\w+)",
                content,
                re.MULTILINE,
            ):
                count += 1
                names.append(m.group(1))
            for m in re.finditer(
                r"^\s*public\s+(?:final\s+|abstract\s+|static\s+|synchronized\s+)*(?:[\w<>\[\],?\s]+)\s+(\w+)\s*\(",
                content,
                re.MULTILINE,
            ):
                name = m.group(1)
