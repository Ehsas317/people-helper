...
from .config import (
    CODE_QUALITY_WEIGHT, UNIQUENESS_WEIGHT, DEMAND_SIGNAL_WEIGHT,
    RELEVANCE_WEIGHT, USEFULNESS_WEIGHT, MAINTAINABILITY_WEIGHT,
...
def _compute_code_quality(cand) -> float:
    ...
    if cand.has_tests: score += 2.5
    if cand.has_docstring: score += 1.5
    if cand.internal_imports == 0: score += 1.5
    elif cand.internal_imports == 1: score += 0.5
    if cand.external_imports <= 3: score += 1.5
    elif cand.external_imports <= 5: score += 0.5
    if cand.filename_score > 0: score += 0.5
    if cand.fan_in == 0: score += 0.5
    ...
    # Penalties
    if cand.complexity > 20: score -= 3.0
    elif cand.complexity > 10: score -= 1.5
    elif cand.complexity > 5: score -= 0.5
    if cand.in_cycle: score -= 2.0
    # No docstring AND no tests = -1.5 (mediocre)
    if not cand.has_docstring and not cand.has_tests: score -= 1.5
...
def _compute_relevance(cand) -> float:
    ...
    if cand.dependency_weight == 0: score += 2.0
    elif cand.dependency_weight == 1: score += 0.5
    elif cand.dependency_weight == 3: score -= 2.5
    if cand.api_surface_count >= 3: score += 1.5
    elif cand.api_surface_count == 2: score += 0.5
    elif cand.api_surface_count == 1: score -= 1.0
    elif cand.api_surface_count == 0: score -= 2.5
    if cand.has_project_specific_refs: score -= 2.0
    if 50 <= cand.loc <= 200: score += 0.5
    elif cand.loc < 20: score -= 1.5
    elif cand.loc > 400: score -= 0.5
    if cand.has_tests: score += 0.5
    if cand.has_docstring: score += 0.5
    if cand.internal_imports == 0: score += 0.5
    # Legal signal — no license means extraction is risky
    if not cand.source_has_license: score -= 1.0
    return max(0.0, min(score, 10.0))
...
def _compute_usefulness(cand) -> float:
    ...
    generic_patterns = [
        "slugify", "sanitize", "escape", "encode", "decode", "format",
        "parse", "validate", "convert", "transform", "compress", "decompress",
        "encrypt", "decrypt", "hash", "checksum", "sort", "filter", "search",
        "match", "replace", "split", "join", "merge", "cache", "memoize",
        "retry", "backoff", "timeout", "serialize", "deserialize",
        "case_insensitive", "levenshtein", "distance", "similarity",
    ]
    found_generic = False
    for name in cand.function_names:
        ...
        for pattern in generic_patterns:
            if pattern in nl:
                score += 1.5; found_generic = True; break
        if found_generic: break
    if not found_generic:
        for name in cand.function_names:
            if len(name) > 25 or name.count("_") > 3:
                score -= 1.0; break
    stem = Path(cand.path).stem.lower()
    if stem in {"utils", "helpers", "common", "validators", "sanitizer", "parser",
                 "formatter", "converter", "serializer", "cache", "retry", "auth",
                 "crypto", "hash", "encode", "decode", "structures", "collections"}:
        score += 1.0
    if 50 <= cand.loc <= 300: score += 1.0
    elif cand.loc < 20: score -= 1.5
    if cand.has_tests: score += 0.5
    if cand.api_surface_count >= 3: score += 1.0
    elif cand.api_surface_count == 1: score -= 0.5
    if cand.is_stdlib_only: score += 0.5
    # No function names at all = -1.0 (not useful as a library)
    if cand.api_surface_count == 0: score -= 1.0
    return max(0.0, min(score, 10.0))
...
def _compute_maintainability(cand) -> float:
    ...
    if cand.comment_ratio >= 0.15: score += 2.0
    elif cand.comment_ratio >= 0.05: score += 1.0
    elif cand.comment_ratio == 0 and cand.loc > 30: score -= 1.5
    if cand.has_docstring: score += 1.0
    if cand.complexity > 0:
        if cand.complexity <= 5: score += 1.5
        elif cand.complexity <= 10: score += 0.5
        elif cand.complexity > 20: score -= 2.0
        elif cand.complexity > 15: score -= 1.0
    if 50 <= cand.loc <= 200: score += 1.0
    elif cand.loc > 400: score -= 0.5
    if cand.has_tests: score += 0.5
    return max(0.0, min(score, 10.0))
...
def _compute_uniqueness(similar_count: int) -> float:
    ...
    if similar_count == 0: return 8.0
    elif similar_count <= 2: return 6.0
    elif similar_count <= 5: return 4.0
    else: return 2.0


def _compute_demand_signal(cand) -> float:
    if not cand.similar_projects: return 5.0
    total, weight_sum = 0.0, 0.0
    for i, proj in enumerate(cand.similar_projects):
        w = 1.0 / (i + 1)
        ss = min(10.0, proj.stars / 100.0) if proj.stars > 0 else 0.0
        fs = min(5.0, proj.forks / 50.0) if proj.forks > 0 else 0.0
        is_ = min(3.0, proj.open_issues / 10.0) if proj.open_issues > 0 else 0.0
        rs = 0.0
        if proj.stars > 0 and proj.forks > 0:
            ratio = proj.stars / proj.forks
            rs = 0.0 if ratio > 50 else (1.5 if ratio < 5 else 0.5)
        total += w * (ss + fs + is_ + rs); weight_sum += w
...
def score_candidate(cand, similar_count: int) -> None:
    ...
    raw = (CODE_QUALITY_WEIGHT * cand.code_quality + UNIQUENESS_WEIGHT * cand.uniqueness
           + DEMAND_SIGNAL_WEIGHT * cand.demand_signal + RELEVANCE_WEIGHT * cand.relevance
           + USEFULNESS_WEIGHT * cand.usefulness + MAINTAINABILITY_WEIGHT * cand.maintainability)
    if cand.relevance < 3.0: raw *= 0.5
