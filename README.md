...
- With `--extract`: copies files out, strips license headers, generates package scaffolds
...
1. **Cyclomatic complexity is available for all 13 languages.** Python uses AST-based complexity (most accurate). Other languages use regex-based complexity (counts if/for/while/case/&&/||/? — an approximation but gives fair scores). Fan-in and import cycle detection remain Python-only.
...
4. **The `--extract` feature produces a starting point, not a finished package.** You still need to review the code, adjust imports, add tests, and choose a final license. The extracted package uses MIT by default.

5. **GitHub search has rate limits** (30/min for unauthenticated, 5000/hour with PAT). For large analyses, use `--no-network` and run search separately.
...
└──────────────┘    │  extractable     │    │  • sibling resolution│
...
| **Uniqueness** | 15% | Fewer similar projects on GitHub = higher score (0 results: 8, 1-2: 6, 3-5: 4, 6+: 2). --no-network mode: neutral 5.0 |
...
python people_helper.py --repo your-username/your-repo

# With output path
python people_helper.py --repo your-username/your-repo --output my-report.md

# Verbose mode (see each step)
python people_helper.py --repo your-username/your-repo --verbose

# Local-only (no GitHub search, faster)
python people_helper.py --repo your-username/your-repo --no-network

# Filter by language
python people_helper.py --repo your-username/your-repo --language Python

# Extract top candidates to ./extracted/ (creates package scaffolds)
python people_helper.py --repo your-username/your-repo --extract ./extracted/

# Extract with custom thresholds
python people_helper.py --repo your-username/your-repo --extract ./extracted/ --max-extract 3 --extract-min-score 7.0

# Control output size
python people_helper.py --repo your-username/your-repo --max-candidates 5 --min-stars 10
```
...
- Hard-rejects any PAT with write-capable scopes
...
- Never leaks analyzed-repo details in persisted output
...
Python uses AST-based complexity (most accurate). Other languages use regex-based complexity (counts if/for/while/case/catch/&&/||/?). Fan-in and import cycle detection remain Python-only.
